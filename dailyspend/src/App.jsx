...
const CURRENCIES = [
  {code:"USD",sym:"$"}, {code:"AED",sym:"د.إ"}, {code:"EUR",sym:"€"},  {code:"GBP",sym:"£"},
...
//  Shown on first launch (or when key is cleared). Validates key before saving.
// ─────────────────────────────────────────────────────────────────────────────
function ApiKeySetup({ onSave }) {
  ...
  const test = async () => {
    const k = key.trim();
    if (!k) { setErrMsg("Please paste your API key first."); setStatus("error"); return; }
    if (!k.startsWith("sk-ant-")) { setErrMsg("That doesn't look like an Anthropic key — it should start with sk-ant-"); setStatus("error"); return; }
    setStatus("testing"); setErrMsg("");
    // Temporarily save so callAI can read it
    saveKey(k);
    try {
      await AI.testKey();
      setStatus("ok");
      setTimeout(()=>onSave(k), 600);
    } catch(e) {
      clearKey();
      if (e.message==="INVALID_KEY") {
        setErrMsg("Key rejected by Anthropic — double-check you copied the full key.");
      } else {
        setErrMsg(`Connection error: ${e.message}. Check your internet and try again.`);
      }
      setStatus("error");
    }
  };
  ...
  return (
    <div style={{minHeight:"100vh",background:"#070C18",display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Inter',-apple-system,sans-serif"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap');*{box-sizing:border-box;}input:focus{outline:2px solid rgba(110,231,183,0.5)!important;outline-offset:1px;}button:focus-visible{outline:2px solid rgba(110,231,183,0.6)!important;outline-offset:2px;}`}</style>
      <div style={{width:"100%",maxWidth:440}}>
        {/* Logo */}
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{fontSize:48,marginBottom:12}}>💸</div>
          <div style={{fontSize:24,fontWeight:800,color:"#F8FAFC",letterSpacing:"-0.5px"}}>DailySpend</div>
          <div style={{fontSize:13,color:"#64748B",marginTop:4}}>AI-powered personal expense tracker</div>
        </div>

        {/* Card */}
        <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.09)",borderRadius:20,padding:28}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
            <div style={{width:36,height:36,borderRadius:10,background:"rgba(110,231,183,0.12)",border:"1px solid rgba(110,231,183,0.2)",display:"flex",alignItems:"center",justifyContent:"center",color:"#6EE7B7",flexShrink:0}}><IcoKey/></div>
            <div>
              <div style={{fontSize:14,fontWeight:700,color:"#F8FAFC"}}>Connect your AI</div>
              <div style={{fontSize:11,color:"#64748B",marginTop:1}}>One-time setup — key saved on this device only</div>
            </div>
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20}}>
            {[
              ["1","Go to console.anthropic.com","Sign in or create a free account"],
              ["2","Click API Keys → Create Key","Name it anything, e.g. \"DailySpend\""],
              ["3","Copy the key and paste below","It starts with sk-ant-api03-…"],
            ].map(([n,title,desc])=>(
              <div key={n} style={{display:"flex",gap:10,alignItems:"flex-start",padding:"10px 12px",background:"rgba(255,255,255,0.03)",borderRadius:10,border:"1px solid rgba(255,255,255,0.06)"}}>
                <div style={{width:22,height:22,borderRadius:"50%",background:"rgba(110,231,183,0.15)",border:"1px solid rgba(110,231,183,0.3)",color:"#6EE7B7",fontSize:11,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>{n}</div>
                <div><div style={{fontSize:12,fontWeight:600,color:"#F1F5F9"}}>{title}</div><div style={{fontSize:11,color:"#64748B",marginTop:2}}>{desc}</div></div>
              </div>
            ))}
          </div>

          {/* Key input */}
          <label htmlFor="api-key-input" style={{fontSize:10,fontWeight:700,color:"#6EE7B7",display:"block",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.5px"}}>Your Anthropic API Key</label>
          <div style={{position:"relative",marginBottom:12}}>
            <input
              ...
              style={{width:"100%",background:"rgba(255,255,255,0.06)",border:`1.5px solid ${status==="error"?"rgba(252,165,165,0.5)":status==="ok"?"rgba(110,231,183,0.5)":"rgba(255,255,255,0.1)"}`,borderRadius:10,padding:"12px 44px 12px 14px",color:"#F1F5F9",fontSize:13,outline:"none",fontFamily:"'SF Mono',Monaco,monospace",letterSpacing:"0.03em",minHeight:46}}
            />
            <button onClick={()=>setShow(s=>!s)} aria-label={show?"Hide key":"Show key"}
              style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"#475569",cursor:"pointer",display:"flex",padding:4}}>
              {show?<IcoEyeOff/>:<IcoEye/>}
            </button>
          </div>

          {/* Error / success */}
          {status==="error" && <div role="alert" style={{fontSize:12,color:"#FCA5A5",padding:"8px 12px",background:"rgba(252,165,165,0.08)",border:"1px solid rgba(252,165,165,0.2)",borderRadius:8,marginBottom:12}}>⚠️ {errMsg}</div>}
          {status==="ok"    && <div role="status" style={{fontSize:12,color:"#6EE7B7",padding:"8px 12px",background:"rgba(110,231,183,0.08)",border:"1px solid rgba(110,231,183,0.2)",borderRadius:8,marginBottom:12}}>✅ Key verified! Loading your app…</div>}

          {/* CTA */}
          <button onClick={test} disabled={status==="testing"||status==="ok"}
            style={{width:"100%",padding:"14px",borderRadius:11,border:"none",fontWeight:800,fontSize:14,cursor:status==="testing"||status==="ok"?"not-allowed":"pointer",minHeight:50,transition:"all 0.15s",
              ...
              boxShadow:status==="testing"||status==="ok"?"none":"0 4px 20px rgba(52,211,153,0.25)"}}>
            {status==="testing"?"🔄 Verifying key…":"✅ Save & Start Using DailySpend"}
          </button>

          {/* Privacy note */}
          <div style={{display:"flex",gap:8,marginTop:14,padding:"8px 10px",background:"rgba(255,255,255,0.02)",borderRadius:8,border:"1px solid rgba(255,255,255,0.05)"}}>
            <span style={{fontSize:14,flexShrink:0}}>🔒</span>
            <span style={{fontSize:11,color:"#475569",lineHeight:1.6}}>Your key is stored only in <strong style={{color:"#64748B"}}>your browser's localStorage</strong>. It never leaves your device and is never sent to anyone except Anthropic's API directly.</span>
          </div>
        </div>

        {/* Free tier note */}
        <div style={{textAlign:"center",marginTop:16,fontSize:12,color:"#334155",lineHeight:1.7}}>
          The free Anthropic tier includes enough credits for personal use.<br/>
          <a href="https://console.anthropic.com" target="_blank" rel="noopener noreferrer" style={{color:"#6EE7B7",textDecoration:"none"}}>console.anthropic.com →</a>
        </div>
      </div>
    </div>
  );
...
// ─────────────────────────────────────────────────────────────────────────────
//  API KEY SETTINGS PANEL  (in-app modal to update/remove key)
// ─────────────────────────────────────────────────────────────────────────────
function ApiKeySettings({ onClose, onClear }) {
  ...
  const update = async () => {
    ...
    try {
      ...
      clearKey();
      setErrMsg(e.message==="INVALID_KEY"?"Key rejected by Anthropic — check it's correct.":"Connection error. Try again.");
      setStatus("error");
    }
  };

  const remove = () => { clearKey(); onClear(); onClose(); };
...
function NLPLogger({ onAdd, currency, nextId, cats }) {
  ...
  const send=async()=>{const t=txt.trim();if(!t||busy)return;setTxt("");setMicErr("");setMsgs(m=>[...m,{r:"user",t}]);setBusy(true);scroll();
    ...
    catch(e){setMsgs(m=>[...m,{r:"bot",t:e.message==="INVALID_KEY"?"⚠️ API key is invalid. Tap the ⚙️ Settings icon to update it.":"Sorry, couldn't parse that. Try including an amount and a description."}]);}
...
function ForecastPanel({ expenses, budgets, currency, cats }) {
  ...
  const run=async()=>{if(expenses.length<2){setErr("Add more expenses for a meaningful forecast.");setSt("error");return;}setSt("loading");setErr("");try{setFc(await AI.forecast(expenses,budgets,currency,catNames));setSt("done");}catch(e){setErr(e.message==="INVALID_KEY"?"Your API key is invalid. Go to ⚙️ Settings to update it.":e.message||"Forecast failed.");setSt("error");}};
...
export default function App() {
  ...
  // API key gate — skip if running in Claude artifact environment
  const [apiKey, setApiKey] = useState(() => CLAUDE_ENV ? "CLAUDE_ENV" : getKey());
  const hasKey = CLAUDE_ENV || !!apiKey;
  ...
  // Show key setup screen if no key and not in Claude env
  if (!hasKey) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#070C18",fontFamily:"'Inter',-apple-system,sans-serif",paddingBottom:80,color:"#F1F5F9"}}>
      ...
      {showKeySt  && <ApiKeySettings onClose={()=>setShowKeySt(false)} onClear={()=>setApiKey(null)}/>}
